package com.c2h6s.tinkers_advanced.tools.content.misc;
...
public class ToolEnergyProduction {
    ...
    public static void updateProduction(ToolStack toolStack,ToolEnergyProduction production){
        IToolUuidGetter.getUuidForTool(toolStack).ifPresent(uuid ->
                ENERGY_PRODUCTION_MAP.put(uuid,production));

    }

    public static @Nullable ToolEnergyProduction getFromMap(ToolStack toolStack){
        if (IToolUuidGetter.getUuidForTool(toolStack).isPresent()) return ENERGY_PRODUCTION_MAP.get(IToolUuidGetter.getUuidForTool(toolStack).get());
        return null;
    }
