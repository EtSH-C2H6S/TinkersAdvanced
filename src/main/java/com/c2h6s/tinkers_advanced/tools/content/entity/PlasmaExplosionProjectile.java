    ...
    public void tick() {
        ...
        if (this.tickCount ==7&&this.getOwner() instanceof Player player&&!this.level().isClientSide){
            ...
            if (this.level() instanceof ServerLevel serverLevel){
                ...
                serverLevel.sendParticles((ParticleOptions) TiAcCrParticleTypes.SPARK_BLUE.get(),this.getX(),this.getY(),this.getZ(),(int)Math.min(30*scale,30),0.025*scale,0.025*scale,0.025*scale,0.4*scale);
                ...
                else serverLevel.playSeededSound(null,this.getX(),this.getY(),this.getZ(), SoundEvents.FIREWORK_ROCKET_BLAST, SoundSource.PLAYERS,1,1, TinkersAdvanced.RANDOM.nextLong());
            }
        }
    }
}
