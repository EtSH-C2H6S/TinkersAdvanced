...
import com.c2h6s.tinkers_advanced.tools.capability.ElectronTunerItemCapabilityProvider;
...
import com.c2h6s.tinkers_advanced.core.library.capability.IExchangerTickable;
...
public class ElectronTunerItem extends ModifiableSwordItem {
    ...
    public static void generatorTick(ItemStack stack, Level level, @Nullable LivingEntity holderEntity, @Nullable BlockEntity holderBlockEntity){
        ...
        if (production.needUpdate()) {
            ...
            for (ModifierEntry modifier : tool.getModifierList()) {
                ...
                int amplifiedAmount = (int) (baseAmount * tool.getStats().get(EtSTLibToolStat.POWER_MULTIPLIER));
    ...
    public List<Component> getStatInformation(IToolStackView tool, @org.jetbrains.annotations.Nullable Player player, List<Component> tooltips, TooltipKey key, TooltipFlag tooltipFlag) {
        if (player != null) {
            tooltips.add(Component.translatable("tool_stat.tinkers_advanced.generate_factor").append("§4" + tool.getStats().get(EtSTLibToolStat.POWER_MULTIPLIER)));
        }
        return super.getStatInformation(tool, player, tooltips, key, tooltipFlag);
    }
    protected static final IExchangerTickable EXCHANGER_TICKABLE = ElectronTunerItem::generatorTick;
    ...
    public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag nbt) {
        if (stack.getItem() instanceof ElectronTunerItem) return new ToolCapabilityProvider(stack){
            ...
            public <T> LazyOptional<T> getCapability(Capability<T> cap, @Nullable Direction side) {
                ...
                } else if (cap== TiAcCapabilities.EXCHANGER_TICKABLE){
                    return LazyOptional.of(()->EXCHANGER_TICKABLE).cast();
                }
