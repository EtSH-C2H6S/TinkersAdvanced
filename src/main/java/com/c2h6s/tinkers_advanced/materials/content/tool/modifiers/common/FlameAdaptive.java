package com.c2h6s.tinkers_advanced.materials.content.tool.modifiers.common;
...
public class FlameAdaptive extends EtSTBaseModifier {
    ...
    public void postMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damageDealt,float damage) {
        if (context.isFullyCharged()&&context.getTarget() instanceof LivingEntity living){
            living.hurt(LegacyDamageSource.any(living.damageSources().lava().typeHolder(),context.getAttacker()).setBypassInvulnerableTime(),modifier.getLevel()*2);
        }
    }
    ...
    public void afterArrowHit(ModDataNBT persistentData, ModifierEntry entry, ModifierNBT modifiers, AbstractArrow arrow, @Nullable LivingEntity attacker, @NotNull LivingEntity target, float damageDealt) {
        if (arrow.isCritArrow()&&attacker!=null){
            target.hurt(LegacyDamageSource.any(target.damageSources().lava().typeHolder(),attacker).setBypassInvulnerableTime(),entry.getLevel()*2);
        }
    }
