        ...
        for (int i =0;i<entry.getLevel();i++) {
            if (!cachedCd) {
                for (ModifierEntry modifierEntry : modifiers) {
                    cachedCd = true;
                    damage = modifierEntry.getHook(EtSTLibHooks.PROJECTILE_DAMAGE).getProjectileDamage(persistentData, modifierEntry, modifiers, arrow,arrow, attacker,target,baseDamage,damage);
                }
                cachedCd = false;
            }
        }
